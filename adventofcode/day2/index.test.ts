...
describe('day2',()=>{
    ...
    it('If there are more than 12 red should return not valid',()=>{
        expect(isPossibleGame('Game 30: 13 red')).toStrictEqual({id:30,isPossible:false})
    })
    it('If there are more than 13 green should return not valid',()=>{
        expect(isPossibleGame('Game 30: 14 green')).toStrictEqual({id:30,isPossible:false})
    })
    it('If there are more than 14 blue should return not valid',()=>{
        expect(isPossibleGame('Game 30: 15 blue')).toStrictEqual({id:30,isPossible:false})
    })
    it('If there are one valid turn should return valid',()=>{
        expect(isPossibleGame('Game 30: 12 red, 13 green, 14 blue')).toStrictEqual({id:30,isPossible:true})
    })
    it('If there are two valid turns should return valid',()=>{
        expect(isPossibleGame('Game 30: 12 red, 13 green, 14 blue; 12 red, 13 green, 14 blue')).toStrictEqual({id:30,isPossible:true})
    })
})
