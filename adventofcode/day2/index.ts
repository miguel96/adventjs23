...
export const isPossibleGame = (
    ...
    const isPossible = turns.split(';').filter(Boolean).every(turn=>{
        let turnInfo = turn;
        const data = {};
        let match;
        let isValid=true;
        do {
            const [,count,color] = /([0-9]+) (blue|green|red)/.exec(turnInfo)||[]
...
export const getGamePower = (
    gameInfo: string
):number=>{
    const [,gameId,turns]=gameInfo.match(/Game ([0-9]+): (.*)/)||[];
    const resp = turns.split(';').filter(Boolean).reduce<Record<string,number>>((data,turn)=>{
        let turnInfo = turn;
        let match;
        do {
            const [,count,color] = /([0-9]+) (blue|green|red)/.exec(turnInfo)||[]
